...
T = {
    ...
    **Scopo della ricerca:** Lo studio si propone di analizzare la correlazione tra la valutazione umana di testi creativi (scritti sia da esseri umani che da LLM) e la valutazione automatica effettuata tramite approccio LLM-as-a-judge (l'uso di LLM per giudicare testi basandosi su criteri predefiniti). L'obiettivo finale è studiare l'allineamento tra le metriche di giudizio umane e quelle generate dai modelli linguistici.
    ...
    **Volontarietà e Recesso:** La partecipazione è strettamente volontaria. Puoi abbandonare la sessione in qualsiasi momento chiudendo il browser senza alcuna conseguenza. Tuttavia, data la natura completamente anonima dei dati, una volta inviato il questionario **non sarà possibile richiederne la cancellazione**, in quanto non saremo in grado di associare le risposte alla tua identità.
    ...
    **Research Purpose:** This study aims to analyze the correlation between human evaluation of creative texts (written by both humans and LLMs) and automatic evaluation performed using the *LLM-as-a-judge* approach (using LLMs to judge texts based on predefined criteria). The ultimate goal is to study alignment between human judgment metrics and those generated by language models.
    ...
    **Voluntary Participation and Withdrawal:** Participation is strictly voluntary. You may leave the session at any time by closing your browser without any consequences. However, given the completely anonymous nature of the data, once the questionnaire is submitted, **it will not be possible to request its deletion**, as we will be unable to associate the responses with your identity.
